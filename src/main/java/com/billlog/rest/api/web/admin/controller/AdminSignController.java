package com.billlog.rest.api.web.admin.controller;
...
public class AdminSignController {
    ...
    public ModelAndView adminLoginPage(Model model, ModelAndView mav, @ModelAttribute FoodUser foodUser) {
    ...
    public ModelAndView premiumWatingUsers(Model model, ModelAndView mav, @ModelAttribute FoodUser foodUser) {

        List<FoodUser> users = userMapper.getAll("P","W");
    ...
    public String adminLoginRegister(Model model, ModelAndView mav, @Valid FoodUser foodUser) {

        String url = "";

        FoodUser user = userMapper.findByUsername(foodUser.getUser_id());
...
//            mav.setViewName("redirect:/web/admin/login");
            ...
            //패스워드가 다를 경우.
            if (!passwordEncoder.matches(foodUser.getUser_pwd(), user.getUser_pwd())) {
