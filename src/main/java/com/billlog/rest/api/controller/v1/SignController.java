...
import com.billlog.rest.api.advice.exception.*;
...
public class SignController {
    ...
    public SingleResult<String> signin(@ApiParam(value = "회원ID", required = true) @RequestParam String id,
        ...
        FoodUser user = userMapper.findByUsername(id);
    ...
    public SingleResult<String> signupFindId(@ApiParam(value = "회원 ID", required = true) @RequestParam String user_id,
        ...
        FoodUser userFindById = userMapper.findByUsername(user_id);
    ...
    public CommonResult signin(@ApiParam(value = "회원ID", required = true) @RequestParam String id,
                               ...
                               @ApiParam(value = "지역") @RequestParam(value="city", required=false) int city,
                               @ApiParam(value = "회사명") @RequestParam(value="team", required=false) String team,
                               @ApiParam(value = "서비스 제공자") @RequestParam(value="provider", required=false) String provider) {

        FoodUser foodUser = new FoodUser();
        foodUser.setUser_id( id );
        foodUser.setUser_name(name);
        foodUser.setUser_pwd( passwordEncoder.encode(password) );
        foodUser.setUser_email( email );
        foodUser.setUser_gender( gender ); //F or M
        foodUser.setUser_city( city ); // Only Number
        foodUser.setUser_team( ("".equals(team) == true) ? null : team ); // String
        foodUser.setUser_role( 3 ); // Default : 3 ( ROLE _USER )

        if(!"".equals(provider) && provider != null) {
            foodUser.setProvider(provider);
        }


        userMapper.createUser(foodUser);
    ...
    public CommonResult signupProvider(@ApiParam(value = "서비스 제공자 provider", required = true, defaultValue = "kakao") @PathVariable String provider,
        ...
        Optional<FoodUser> user = Optional.ofNullable(userMapper.findByUidAndProvider(String.valueOf(profile.getId()), provider));
