...
public class CommentController {
    ...
    public ListResult<FoodComment> getCommentsByCommentIdx(@ApiParam(value = "댓글 관리 IDX", required = true)@PathVariable int comment_manage_idx) {
    /*
...
//        page = (page - 1) * count;
    ...
    @PostMapping("/comment")
    public CommonResult createComment(@ApiParam(value = "댓글 정보 Object", required = true) FoodComment foodComment) throws UnsupportedEncodingException {
        int result = 0;

        result = commentMapper.createComment(foodComment);

        if (result == 1) {

            // 커뮤니티 게시글 작성자에게 댓글이 등록되었다고 FCM push를 전송한다.
            // FCM 코멘트 IDX 값을 이용하여 글 커뮤니티 글 작성자의 user_idx를 찾아내 토큰 값을 가져온다.
            String token = deviceMapper.findMsgReciverUserByCommentIdx(foodComment.getComment_idx());


            Map<String, Object> fcm = new HashMap<>();

            String title = "FoodCorner";
    ...
    public CommonResult modifyCommentByIdxSn(@ApiParam(value = "댓글 IDX", required = true)@PathVariable int comment_idx,
        ...
        foodComment.setComment_idx(comment_idx);
        foodComment.setComment_sn(comment_sn);
        foodComment.setWriter_user_idx(writer_user_idx);

        if(!"".equals(content) || content != null) {
            foodComment.setContent(content);
        }

        int result = commentMapper.modifyCommentByIdxSn(foodComment);
