...
import com.billlog.rest.api.advice.exception.CUserDeleteException;
import com.billlog.rest.api.advice.exception.CUserModifyException;
import com.billlog.rest.api.advice.exception.CUserNotFoundException;
import com.billlog.rest.api.config.security.JwtTokenProvider;
...
import com.billlog.rest.api.util.MailHandler;
import com.billlog.rest.api.util.TempKey;
...
@Api(tags = {"2. User"})
@RestController //결과 데이터를 json으로 보내기
...
public class UserController {
    ...
    public SingleResult<FoodUser> requestMeByToken(@RequestHeader("X-AUTH-TOKEN") String token)  {

        FoodUser loginUser = (FoodUser) userDetailsService.loadUserByUsername(jwtTokenProvider.getUserPk(token));
    ...
    public ListResult<FoodUser> findAllUser(@ApiParam(value = "타입")@RequestParam(value="type", required=false) String type,
    ...
    public SingleResult<FoodUser> findUserByIdx(@ApiParam(value = "회원IDX", required = true)@PathVariable int user_idx) {
        FoodUser user = userMapper.getUserById(user_idx);
    ...
    public CommonResult modifyUserById(@ApiParam(value = "회원IDX", required = true)@PathVariable int user_idx,
                                       ...
                                       @ApiParam(value = "수정할 회원정보 Object", required = true) FoodUser user,
        ...
        if(!CustomUtils.isEmpty(file)){
            ...
            FoodUser userTemp = userMapper.getUserById(user_idx);
    ...
    public CommonResult eamilAuth(@ApiParam(value = "회원 id", required = true) @RequestParam int user_idx,
        ...
        if(user.getUser_email().equals(user_email)){
            ...
            if (chkVal) {
                ...
                sendMail.setSubject("[FOODCORNER 이메일 인증입니다.]");
                sendMail.setText(new StringBuffer().append("<h1>FOODCORNER 메일인증 서비스</h1>")
                        .append("<h3>안녕하세요. FOODCORNER 입니다. 회원님의 이메일인증 확인을 위하여 발송된 메일입니다.</h3>")
                        ...
                        .toString());
                sendMail.setFrom(official_email, "foodcorner");
