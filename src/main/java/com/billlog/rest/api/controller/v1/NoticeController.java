...
import com.billlog.rest.api.mapper.NoticeMapper;
...
public class NoticeController {
    ...
    @GetMapping("/notices")
    public ListResult<FoodNotice> getAll(@ApiParam(value = "페이지 번호", required = true)@RequestParam int page,
    ...
    public SingleResult<FoodNotice> getNoticeByIdx(@ApiParam(value = "공지사항글 IDX", required = true)@PathVariable int notice_idx) {

        FoodNotice notice = noticeMapper.getNoticeByIdx(notice_idx);
    ...
    @PostMapping("/notice")
    public CommonResult createNoticeArticle(FoodNotice foodNotice) throws UnsupportedEncodingException {

        int result = noticeMapper.createNoticeArticle(foodNotice);
    ...
    public CommonResult modifyNoticeArticleByIdx(@ApiParam(value = "공지사항 IDX", required = true)@PathVariable int notice_idx,
                                                    @ApiParam(value = "공지사항 작성 Object", required = true) FoodNotice foodNotice){

        //notice 인덱스 번호가 없을 경우 메시지와 함께 예외처리
        if("".equals(foodNotice.getNotice_idx())){
            throw new CInfoArticleNotFoundException();
        }

        int result = noticeMapper.modifyNoticeArticleByIdx(foodNotice);
